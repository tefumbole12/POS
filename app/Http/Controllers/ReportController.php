...
class ReportController extends Controller
{
    ...
    public function dailyBooking($year, $month)
    {
        ...
        if($role->hasPermissionTo('booking_report')){
            ...
            while($start <= $number_of_day)
            {
                $booking_data_array[$start] = [];
                if($start < 10)
                    $date = $year.'-'.$month.'-0'.$start;
                else
                    $date = $year.'-'.$month.'-'.$start;

                $booking_data = BookingProduct::with('booking')
                    ->whereDate('start', '<=', $date)
                    ->whereDate('end', '>=', $date)
                    ->get();

                foreach ($booking_data as $booking) {
                    if(in_array(@$booking->booking->customer->name, @$booking_data_array[$start]) == false) {
                        $booking_data_array[$start][] = @$booking->booking->customer->name;
                    }
                    if (!isset($color[$start])) {
                        if($booking->booking->payment_status == 1 || $booking->booking->payment_status == 2 || $booking->booking->payment_status == 3) {
                            $color[$start] = 'green';
                        }
                        if($booking->booking->payment_status == 4) {
                            $color[$start] = 'red';
                        }
                    } else {
            ...
            $lims_warehouse_list = Warehouse::where('is_active', true)->get();
            $warehouse_id = 0;
            return view('booking.daily_booking', compact('color', 'booking_data_array', 'start_day', 'year', 'month', 'number_of_day', 'prev_year', 'prev_month', 'next_year', 'next_month', 'lims_warehouse_list', 'warehouse_id'));
        }
        else
            return redirect()->back()->with('not_permitted', 'Sorry! You are not allowed to access this module');
    }
